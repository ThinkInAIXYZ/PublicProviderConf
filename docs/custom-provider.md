...
The catalog contains 44 selected chat, coding, reasoning, and frontier models across nine sources. It is designed to be used before AIHubMix, while AIHubMix remains available as the broader lower-tier fallback source. Inclusion provides capability metadata; it does not guarantee that an official or third-party endpoint serves the model.

## Selected Models

The maintained selection is dated September 20, 2026. Model IDs are exact API identifiers; product names and release dates are not synthesized into additional aliases.
